...
var refreshtabwindow = function(tabTitle,url){
	refreshTab(tabTitle,url,'icon icon-null');
};

var closetabwindow=function(){
	closeTab();
};

var closemenu=function(title){
...
var refreshmenu=function(menus_var){
    ...
    $.each(menus_var.menus, function(j, o) {//二层处理二级菜单
    	if(j==0){
    		tabTitle=o.menuname;
    		url=o.url;
    		icon=o.icon;
		...
		menulist += '<li><div><a title="'+o.menuname+'" ref="'+o.menuid+'" href="#" rel="' + o.url + '" icon="' + o.icon + '"  ><span class="icon_ks '+o.icon+'" >&nbsp;</span><span class="nav">' + o.menuname + '</span></a></div></li> ';
    });
    menulist += '</ul>';
	//alert($("#nav >ul"));
	$('#nav').accordion('add', {
        title: menus_var.menuname,
        content: menulist,//二级菜单的内容
        iconCls: 'icon ' + menus_var.icon
    });
...
function refreshTab(subtitle,url,icon){
