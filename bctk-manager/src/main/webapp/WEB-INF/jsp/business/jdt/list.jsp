...
var fixWidth=function(percent){
	...
	var columns_v = [ [  {
		...
		formatter : function(value, row, index) {
			return "<a href=javascript:editjdt('"+row.uuid+"')>查看</a>";
		}
	},{
		...
		width : fixWidth(0.05),
		formatter : function(value, row, index) {
			return "<a href=javascript:deletejdt('"+row.uuid+"')>删除</a>";
		}
	} ] ];
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("添加题目信息", width*0.6, height*0.5, '${baseurl}jdtInput.action');
		}
	} ];
	...
	function editjdt(id){
		...
		//打开修改窗口
		createmodalwindow("题目详细信息", width*0.6, height*0.6, '${baseurl}jdtEdit.action?uuid='+id);
	}
	//删除用户方法
	function deletejdt(id){
	...
	<TABLE class="table_search" align=center>
		<TBODY>
			...
			<TR>
				...
				<td ><INPUT class="easyui-textbox" type="text" name="jdtCustom.teachername" class="one"/></TD>
				...
				<TD class="left">答案：</TD>
				<td><INPUT type="text" class="easyui-textbox" name="jdtCustom.answer"/></TD>
				<td>
...
<form id="jdtdeleteform" action="${baseurl}jdt/delete.action" method="post">
