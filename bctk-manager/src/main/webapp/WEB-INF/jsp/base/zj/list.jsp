...
var fixWidth=function(percent){
	...
	var columns_v = [ [ {
		...
		formatter : function(value, row, index) {
			return "<a href=javascript:editzj('"+row.uuid+"')>修改</a>";
		}
	},{
		...
		width : fixWidth(0.05),
		formatter : function(value, row, index) {
			return "<a href=javascript:deletezj('"+row.uuid+"')>删除</a>";
		}
	} ] ];
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("添加章节信息", width*0.3, height*0.6, '${baseurl}zjInput.action');
		}
	} ];
	...
	$(function() {
		$('#zjlist').datagrid({
			...
			striped : true,//条纹显示效果
			url : '${baseurl}zj/query.action',//加载数据的连接，引连接请求过来是json数据
	    ...
	    $('#kcid').combobox({
	        url: '${baseurl}kc/jsonList.action',
	        valueField: 'name',
	        textField: 'name',
	        editable: true,
	        mode:'remote',
	    ...
	    $('#zjname').combobox({
	        url: '${baseurl}zj/jsonList.action',
	        valueField: 'name',
	        textField: 'name',
	        editable: true,
	        mode:'remote',
	    ...
	    $('#kcid').combobox({
	    	onSelect: function(data){
	    		...
	    		//根据课程名称查询对应章节信息
	    	    $('#zjname').combobox('reload','${baseurl}zj/jsonList.action?zjCustom.kcname='+kcname);
	    	}
	    });
	...
	function queryzj(){
		...
		var formdata = $("#zjqueryForm").serializeJson();
		$('#zjlist').datagrid('load',formdata);
	}
	//修改章节
	function editzj(id){
		
		//打开修改窗口
		createmodalwindow("修改章节信息", width*0.3, height*0.6, '${baseurl}zjEdit.action?uuid='+id);
	}
	//删除章节方法
	function deletezj(id){
		...
		_confirm('您确认删除吗？',null,function (){
			...
			jquerySubByFId('zjdeleteform',zjdel_callback,null,"json");
			
			
		});
	}
	//删除章节的回调
	function zjdel_callback(data){
		message_alert(data);
		//刷新 页面
		//在提交成功后重新加载 datagrid
		//取出提交结果
...
<body>
  ...
  <form id="zjqueryForm">
	...
	<TABLE class="table_search" align="center">
		<TBODY >
			<TR>
				<TD class="left">章节名称：</td>
				<td>
					<input  name="zjCustom.name" id="zjname">
				</TD>
				<TD class="left">课程名称：</td>
				<td>
					<input  name="zjCustom.kcname" id="kcid">
				</TD>
				<td><a id="btn" href="#" onclick="queryzj()"
	...
	<TABLE border=0 cellSpacing=0 cellPadding=0 width="99%" align=center>
		<TBODY  align="center">
			<TR>
				<TD>
					<table id="zjlist"></table>
				</TD>
			</TR>
		</TBODY>
	</TABLE>
</form>
<form id="zjdeleteform" action="${baseurl}zj/delete.action" method="post">
